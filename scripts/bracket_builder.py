...
def roundsPerBracket(playerList):
    if len(playerList) == 3:
        rounds_per_bracket = 3
    if len(playerList) == 4:
        rounds_per_bracket = 6
    if len(playerList) == 5 or len(playerList) == 6:
        rounds_per_bracket = 5
    if len(playerList) == 7 or len(playerList) == 8:
        rounds_per_bracket = 7
    return rounds_per_bracket

# Determines the number of matches per round based on player count
def matchesPerRound(playerList):
    if len(playerList) == 3 or len(playerList) == 4:
        matches_per_round = 1
    if len(playerList) == 5:
        matches_per_round = 2
    if len(playerList) == 6 or len(playerList) == 7:
        matches_per_round = 3
    if len(playerList) == 8:
        matches_per_round = 4
